...
from main.models import BookCopiesModel
from .. import db

class BookCopy(Resource):
    def get(self, id):
        book_copy = db.session.query(BookCopiesModel).get_or_404(id)
...
class BookCopies(Resource):
    def get(self):
        books = db.session.query(BookCopiesModel).all()
        return jsonify([book.to_json() for book in books])

    def post(self):
        book_copy = BookCopiesModel.from_json(request.get_json())
        try:
            db.session.add(book_copy)
            db.session.commit()
